...
function MistakeImage({ src, alt, icon }) {
  return (
    <div className="mt-5 overflow-hidden rounded-3xl bg-white/80 p-3 shadow-md dark:bg-white/10">
      {hasRealImage(src) ? (
        <img
          src={src}
          alt={alt}
          className="h-56 w-full rounded-[1.2rem] bg-white object-contain shadow-sm"
        />
      ) : (
        <div className="flex h-56 w-full flex-col items-center justify-center rounded-[1.2rem] bg-white text-center shadow-sm dark:bg-white/10">
          <div className="text-6xl">{icon}</div>

          <p className="mt-4 px-4 text-base font-black leading-7 text-slate-600 dark:text-slate-300">
  ...
  return (
    <section className="relative overflow-hidden py-10">
      ...
      <div className="page-container">
        {/* Hero Section */}
        <div className="overflow-hidden rounded-4xl border border-white/70 bg-white/90 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-white/5">
          <div className="grid items-center gap-8 p-6 md:p-8 lg:grid-cols-[1fr_0.95fr]">
            {/* Text */}
            <div className="text-right">
              <div className="inline-flex rounded-full bg-red-500/10 px-5 py-2 text-sm font-black text-red-700 dark:text-red-300">
                ⚠️ درس تعليمي
              </div>
              ...
              <p className="mt-6 max-w-3xl text-lg font-bold leading-9 text-slate-600 dark:text-slate-300">
                اختيار الألوان بطريقة غير مناسبة قد يجعل التصميم مزعجًا أو
                صعب القراءة. في هذا الدرس سنتعرف على أشهر الأخطاء وكيف نتجنبها
                بطريقة بسيطة وواضحة.
              </p>
            </div>
            ...
            <div className="rounded-4xl bg-linear-to-br from-blue-50 via-violet-50 to-pink-50 p-4 shadow-inner dark:from-white/10 dark:via-white/5 dark:to-white/10">
              <img
                ...
                className="h-96 w-full rounded-3xl bg-white object-contain p-2 shadow-xl"
              />
            </div>
          </div>
        </div>

        {/* Goals Under The Full Hero */}
        <div className="mt-8 rounded-4xl border border-white/70 bg-white/90 p-6 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-white/5 md:p-8">
          <div className="text-center">
            <div className="mx-auto inline-flex rounded-full bg-green-500/10 px-5 py-2 text-sm font-black text-green-700 dark:text-green-300">
              🎯 أهداف الدرس
            </div>
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-3">
            {lessonGoals.map((goal, index) => (
              <div
                key={goal}
                className="flex h-full items-start gap-4 rounded-3xl bg-blue-50/80 p-5 shadow-md dark:bg-white/10"
              >
        ...
        {/* Intro */}
        <div className="mt-8 rounded-4xl border border-white/70 bg-white/90 p-6 text-center shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-white/5 md:p-8">
        ...
        <div className="mt-8 rounded-4xl border border-white/70 bg-white/90 p-6 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-white/5 md:p-8">
          ...
          <div className="grid auto-rows-fr gap-6 md:grid-cols-2">
            {mistakes.map((item) => (
              <div
                key={item.title}
                className={`relative flex h-full flex-col rounded-4xl border border-white/80 ${item.bg} p-6 pt-24 shadow-lg transition duration-300 hover:-translate-y-1 hover:shadow-2xl dark:border-white/10 dark:bg-white/5`}
              >
                {/* Number Right */}
                <div
                  className={`absolute right-6 top-6 rounded-full px-5 py-3 text-sm font-black shadow-md ${item.badge}`}
                >
                  خطأ {item.number}
                </div>

                {/* Icon Left */}
                <div className="absolute left-6 top-6 grid h-16 w-16 place-items-center rounded-2xl bg-white text-4xl shadow-md dark:bg-white/10">
                  {item.icon}
                </div>
                ...
                <div className="mt-auto pt-5">
                  <div className="rounded-3xl bg-white/85 p-5 text-base font-black leading-8 text-slate-800 shadow-sm dark:bg-white/10 dark:text-white">
        ...
        <div className="mt-8 rounded-4xl border border-white/70 bg-white/90 p-6 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-white/5 md:p-8">
          ...
          <div className="mt-7 grid gap-5 md:grid-cols-3">
            {reminders.map((item) => (
              <div
                key={item.text}
                className="rounded-3xl bg-white/80 p-6 text-center shadow-md dark:bg-white/10"
              >
        ...
        <div className="mt-8 rounded-4xl border border-white/70 bg-white/90 p-6 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-white/5 md:p-8">
          <div className="mb-7 text-center">
            ...
            <p className="mx-auto mt-3 max-w-3xl text-base font-bold leading-8 text-slate-600 dark:text-slate-300">
              فيديو تعليمي قصير لمساعدة الطالب على فهم أخطاء اختيار الألوان بشكل
              أوضح.
            </p>
          </div>
          ...
          {videoUrl ? (
            <div className="mx-auto max-w-5xl overflow-hidden rounded-4xl bg-white p-3 shadow-xl dark:bg-white/10">
              <iframe
                ...
                className="h-72 w-full rounded-3xl bg-black shadow-md md:h-105"
